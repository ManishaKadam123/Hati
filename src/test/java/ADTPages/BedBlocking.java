...
public class BedBlocking 
{
	...
	public void Bed_Blocking_Screen(String ReleaseTimePar,String RemarksPar) throws InterruptedException, IOException 
	{
		...
		FileUtils.copyFile(screenshot, new File(".//ScreenShot//BedBlockingForm.png"));  
		...
		FileUtils.copyFile(screenshot1, new File(".//ScreenShot//BedBlockingList.png")); 
		...
		Thread.sleep(2000);
		//Failure Assertions
				if (driver.getPageSource().contains(Bed)&& driver.getPageSource().contains("ACTIVE")) 
				{
				...
				else
				{
					...
					FileUtils.copyFile(screenshot2, new File(".//ScreenShotFailedTestCase//FailedFor_BedBlocking"+ dateFormat.format(date)));
					Assert.assertTrue(false,"bed Has been not blocked");
				}
	}
